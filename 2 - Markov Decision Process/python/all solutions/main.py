...
def plot_results(solvers, solver_names, figname):
    ...
    indices = []
    for i in range(b.n):
    	indices.append(b.probas[i])
    ax2.plot(range(b.n), indices, 'k--', markersize=12)
    for s in solvers:
    	indices2 = []
    	for i in range(b.n):
    	    indices2.append(s.estimated_probas[i])
    	ax2.plot(range(b.n), indices2, 'x', markeredgewidth=2)
